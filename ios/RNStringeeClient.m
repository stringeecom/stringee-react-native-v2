...
RCT_EXPORT_METHOD(createClientWrapper
                  : (NSString *)uuid baseUrl
                  : (NSString *)baseUrl addresses
                  : (NSArray *)addresses stringeeXBaseUrl
                  : (NSString *)stringeeXBaseUrl) {
    RNClientWrapper *clientWrapper = [RNStringeeInstanceManager.instance.clientWrappers objectForKey:uuid];
    ...
    RNClientWrapper *newClientWrapper = [[RNClientWrapper alloc] initWithIdentifier:uuid
                                                                   ...
                                                                   stringeeXBaseUrl:stringeeXBaseUrl];
...
RCT_EXPORT_METHOD(connect : (NSString *)uuid token : (NSString *)token) {
    ...
    if (wrapper == nil) {
        RCTLogError(@"StringeeError | Wrapper is not found");
        return;
    }
...
RCT_EXPORT_METHOD(registerPushForDeviceToken
                  : (NSString *)uuid deviceToken
                  : (NSString *)deviceToken isProduction
                  : (BOOL)isProduction isVoip
                  : (BOOL)isVoip callback
    ...
    [wrapper.client registerPushForDeviceToken:deviceToken
...
RCT_EXPORT_METHOD(registerPushAndDeleteOthers
                  : (NSString *)uuid deviceToken
                  : (NSString *)deviceToken isProduction
                  : (BOOL)isProduction isVoip
                  : (BOOL)isVoip packageNames
                  : (NSArray *)packageNames callback
                  : (RCTResponseSenderBlock)callback) {
    RNClientWrapper *wrapper = [RNStringeeInstanceManager.instance.clientWrappers objectForKey:uuid];
    if (wrapper == nil) {
        callback(@[ @(NO), @(-1), @"Wrapper is not found" ]);
...
RCT_EXPORT_METHOD(unregisterPushToken : (NSString *)uuid deviceToken : (NSString *)deviceToken callback : (RCTResponseSenderBlock)callback) {
    ...
    [wrapper.client unregisterPushForDeviceToken:deviceToken
                               completionHandler:^(BOOL status, int code, NSString *message) {
        callback(@[ @(status), @(code), message ]);
    }];
}

RCT_EXPORT_METHOD(sendCustomMessage
                  : (NSString *)uuid userId
                  : (NSString *)userId message
    ...
    NSDictionary *data = [NSJSONSerialization JSONObjectWithData:objectData options:NSJSONReadingMutableContainers error:&jsonError];
    ...
    [wrapper.client sendCustomMessage:data
                             toUserId:userId
                    completionHandler:^(BOOL status, int code, NSString *message) {
        callback(@[ @(status), @(code), message ]);
    }];
...
//        [self sendEventWithName:didConnect body:@{ @"userId" : stringeeClient.userId, @"projectId" : stringeeClient.projectId, @"isReconnecting" :
//        @(isReconnecting) }];
//    }
//}
//
...
//        [self sendEventWithName:didDisConnect body:@{ @"userId" : stringeeClient.userId, @"projectId" : stringeeClient.projectId, @"isReconnecting"
//        : @(isReconnecting) }];
//    }
//}
//
...
//
//        [self sendEventWithName:incomingCall body:@{ @"userId" : returnUserId, @"callId" : returnCallId, @"from" : returnFrom, @"to" : returnTo,
//        @"fromAlias" : returnFromAlias, @"toAlias" : returnToAlias, @"callType" : @(index), @"isVideoCall" : @(stringeeCall.isVideoCall),
...
RCT_EXPORT_METHOD(createConversation
                  : (NSString *)uuid userIds
                  : (NSArray *)userIds options
    ...
    [wrapper.client createConversationWithName:name
                                  participants:users
                                       options:convOptions
...
RCT_EXPORT_METHOD(getConversationById : (NSString *)uuid conversationId : (NSString *)conversationId callback : (RCTResponseSenderBlock)callback) {
    ...
    [wrapper.client getConversationWithConversationId:conversationId
                                    completionHandler:^(BOOL status, int code, NSString *message, StringeeConversation *conversation) {
        callback(@[ @(status), @(code), message, [RCTConvert StringeeConversation:conversation] ]);
    }];
}

RCT_EXPORT_METHOD(getLocalConversations
                  : (NSString *)uuid count
                  : (NSUInteger)count userId
    ...
    [wrapper.client getLocalConversationsWithCount:count
                                            userId:userId
                                 completionHandler:^(BOOL status, int code, NSString *message, NSArray<StringeeConversation *> *conversations) {
...
RCT_EXPORT_METHOD(getLastConversations : (NSString *)uuid count : (NSUInteger)count callback : (RCTResponseSenderBlock)callback) {
    ...
    [wrapper.client getLastConversationsWithCount:count
                                completionHandler:^(BOOL status, int code, NSString *message, NSArray<StringeeConversation *> *conversations) {
        callback(@[ @(status), @(code), message, [RCTConvert StringeeConversations:conversations] ]);
    }];
}
...
RCT_EXPORT_METHOD(getConversationsAfter
                  : (NSString *)uuid datetime
                  : (NSUInteger)datetime count
                  : (NSUInteger)count callback
                  : (RCTResponseSenderBlock)callback) {
    RNClientWrapper *wrapper = [RNStringeeInstanceManager.instance.clientWrappers objectForKey:uuid];
    if (wrapper == nil) {
        callback(@[ @(NO), @(-1), @"Wrapper is not found", [NSNull null] ]);
    ...
    [wrapper.client getConversationsAfter:datetime
                                withCount:count
                        completionHandler:^(BOOL status, int code, NSString *message, NSArray<StringeeConversation *> *conversations) {
        callback(@[ @(status), @(code), message, [RCTConvert StringeeConversations:conversations] ]);
    }];
...
RCT_EXPORT_METHOD(getAllConversationsAfter
                  : (NSString *)uuid datetime
                  : (NSUInteger)datetime count
                  : (NSUInteger)count callback
                  : (RCTResponseSenderBlock)callback) {
...
RCT_EXPORT_METHOD(getConversationsBefore
                  : (NSString *)uuid datetime
                  : (NSUInteger)datetime count
                  : (NSUInteger)count callback
                  : (RCTResponseSenderBlock)callback) {
    ...
    [wrapper.client getConversationsBefore:datetime
                                 withCount:count
                         completionHandler:^(BOOL status, int code, NSString *message, NSArray<StringeeConversation *> *conversations) {
        callback(@[ @(status), @(code), message, [RCTConvert StringeeConversations:conversations] ]);
    }];
...
RCT_EXPORT_METHOD(getAllConversationsBefore
                  : (NSString *)uuid datetime
                  : (NSUInteger)datetime count
                  : (NSUInteger)count callback
                  : (RCTResponseSenderBlock)callback) {
...
RCT_EXPORT_METHOD(addParticipants
                  : (NSString *)uuid conversationId
                  : (NSString *)conversationId userIds
                  : (NSArray *)userIds callback
                  : (RCTResponseSenderBlock)callback) {
    RNClientWrapper *wrapper = [RNStringeeInstanceManager.instance.clientWrappers objectForKey:uuid];
    if (wrapper == nil) {
        callback(@[ @(NO), @(-1), @"Wrapper is not found", [NSNull null] ]);
    ...
    [wrapper.client getConversationWithConversationId:conversationId
        ...
        [conversation
         ...
         completionHandler:^(BOOL status, int code, NSString *message, NSArray<StringeeIdentity *> *addedUsers) {
            callback(@[ @(status), @(code), message, [RCTConvert StringeeIdentities:addedUsers] ]);
        }];
    }];
}

RCT_EXPORT_METHOD(removeParticipants
                  : (NSString *)uuid conversationId
                  : (NSString *)conversationId userIds
                  : (NSArray *)userIds callback
                  : (RCTResponseSenderBlock)callback) {
    ...
    [wrapper.client getConversationWithConversationId:conversationId
        ...
        [conversation
         ...
         completionHandler:^(BOOL status, int code, NSString *message, NSArray<StringeeIdentity *> *removedUsers) {
            callback(@[ @(status), @(code), message, [RCTConvert StringeeIdentities:removedUsers] ]);
        }];
    }];
}

RCT_EXPORT_METHOD(updateConversation
                  : (NSString *)uuid conversationId
                  : (NSString *)conversationId params
                  : (NSDictionary *)params callback
                  : (RCTResponseSenderBlock)callback) {
    RNClientWrapper *wrapper = [RNStringeeInstanceManager.instance.clientWrappers objectForKey:uuid];
    if (wrapper == nil) {
        callback(@[ @(NO), @(-1), @"Wrapper is not found", [NSNull null] ]);
    ...
    if (![conversationId isKindOfClass:[NSString class]] || !conversationId.length ||
        (![name isKindOfClass:[NSString class]] && ![avatar isKindOfClass:[NSString class]])) {
        callback(@[ @(NO), @(-2), @"Params are invalid." ]);
        return;
    }
    ...
    [wrapper.client getConversationWithConversationId:conversationId
        ...
        [conversation updateWithName:safeName
                        strAvatarUrl:safeAvatar
                   completionHandler:^(BOOL status, int code, NSString *message) {
            callback(@[ @(status), @(code), message ]);
        }];
    }];
...
RCT_EXPORT_METHOD(getConversationWithUser : (NSString *)uuid userId : (NSString *)userId callback : (RCTResponseSenderBlock)callback) {
    ...
    [wrapper.client getConversationForUsers:users
                          completionHandler:^(BOOL status, int code, NSString *message, NSArray<StringeeConversation *> *conversations) {
        if (!conversations) {
...
RCT_EXPORT_METHOD(getLastUnreadConversations : (NSString *)uuid count : (NSUInteger)count callback : (RCTResponseSenderBlock)callback) {
    ...
    [wrapper.client getLastUnreadConversationsWithCount:count
                                             completion:^(BOOL status, int code, NSString *message, NSArray<StringeeConversation *> *conversations) {
        callback(@[ @(status), @(code), message, [RCTConvert StringeeConversations:conversations] ]);
    }];
}

RCT_EXPORT_METHOD(getUnreadConversationsAfter
                  : (NSString *)uuid datetime
                  : (NSUInteger)datetime count
                  : (NSUInteger)count callback
                  : (RCTResponseSenderBlock)callback) {
    ...
    [wrapper.client getUnreadConversationsAfter:datetime
                                      withCount:count
                                     completion:^(BOOL status, int code, NSString *message, NSArray<StringeeConversation *> *conversations) {
        callback(@[ @(status), @(code), message, [RCTConvert StringeeConversations:conversations] ]);
    }];
...
RCT_EXPORT_METHOD(getUnreadConversationsBefore
                  : (NSString *)uuid datetime
                  : (NSUInteger)datetime count
                  : (NSUInteger)count callback
                  : (RCTResponseSenderBlock)callback) {
    ...
    [wrapper.client getUnreadConversationsBefore:datetime
                                       withCount:count
                                      completion:^(BOOL status, int code, NSString *message, NSArray<StringeeConversation *> *conversations) {
        callback(@[ @(status), @(code), message, [RCTConvert StringeeConversations:conversations] ]);
    }];
...
RCT_EXPORT_METHOD(sendMessage : (NSString *)uuid message : (NSDictionary *)message callback : (RCTResponseSenderBlock)callback) {
    ...
    // Lấy về conversation
    [wrapper.client
     getConversationWithConversationId:convId
     completionHandler:^(BOOL status, int code, NSString *message, StringeeConversation *conversation) {
        ...
        switch (type.intValue) {
            ...
            case StringeeMessageTypePhoto: {
                ...
                msgToSend = [[StringeePhotoMessage alloc] initWithFileUrl:filePath
            ...
            case StringeeMessageTypeVideo: {
                ...
                msgToSend = [[StringeeVideoMessage alloc] initWithFileUrl:filePath
                                                             thumbnailUrl:thumbnail
                                                                    ratio:ratio.floatValue
            ...
            case StringeeMessageTypeFile: {
                ...
                msgToSend = [[StringeeFileMessage alloc] initWithFileUrl:filePath
                                                                fileName:filename
            ...
            case StringeeMessageTypeSticker: {
                ...
                if (![category isKindOfClass:[NSString class]] || !category.length || ![name isKindOfClass:[NSString class]] ||
                    !name.length) {
                    callback(@[ @(NO), @(-2), @"Message is invalid." ]);
                    return;
                }
        ...
        [conversation sendMessageWithoutPretreatment2:msgToSend
                                withCompletionHandler:^(BOOL status, int code, NSString *message) {
            callback(@[ @(status), @(code), message ]);
        }];
    }];
}

RCT_EXPORT_METHOD(deleteMessage
                  : (NSString *)uuid conversationId
                  : (NSString *)conversationId msgId
                  : (NSString *)msgId callback
                  : (RCTResponseSenderBlock)callback) {
    RNClientWrapper *wrapper = [RNStringeeInstanceManager.instance.clientWrappers objectForKey:uuid];
    if (wrapper == nil) {
        callback(@[ @(NO), @(-1), @"Wrapper is not found" ]);
    ...
    [wrapper.client getConversationWithConversationId:conversationId
        ...
        [conversation deleteMessageWithMessageIds:@[ msgId ]
                            withCompletionHandler:^(BOOL status, int code, NSString *message) {
            callback(@[ @(status), @(code), message ]);
        }];
    }];
...
RCT_EXPORT_METHOD(pinMessage
    ...
    [wrapper.client getConversationWithConversationId:conversationId
        ...
        // Lấy tin nhắn
        [conversation getMessageWithId:msgId
                            completion:^(BOOL status, int code, NSString *message, StringeeMessage *msg) {
            if (!msg) {
                callback(@[ @(NO), @(-4), @"Message not found." ]);
                return;
            }
            
            [conversation pinMessage:msg
                               isPin:pin
                          completion:^(BOOL status, int code, NSString *message) {
                callback(@[ @(status), @(code), message ]);
            }];
        }];
    }];
...
RCT_EXPORT_METHOD(editMessage
                  : (NSString *)uuid conversationId
                  : (NSString *)conversationId msgId
                  : (NSString *)msgId newContent
                  : (NSString *)newContent callback
                  : (RCTResponseSenderBlock)callback) {
    RNClientWrapper *wrapper = [RNStringeeInstanceManager.instance.clientWrappers objectForKey:uuid];
    if (wrapper == nil) {
        callback(@[ @(NO), @(-1), @"Wrapper is not found" ]);
...
RCT_EXPORT_METHOD(revokeMessage
    ...
    [wrapper.client getConversationWithConversationId:conversationId
        ...
        [conversation getMessageWithId:msgId
            ...
            [conversation revokeMessage:msg
                                deleted:true
                             completion:^(BOOL status, int code, NSString *message) {
                callback(@[ @(status), @(code), message ]);
            }];
...
RCT_EXPORT_METHOD(getLocalMessages
                  : (NSString *)uuid conversationId
                  : (NSString *)conversationId count
                  : (NSUInteger)count callback
                  : (RCTResponseSenderBlock)callback) {
    RNClientWrapper *wrapper = [RNStringeeInstanceManager.instance.clientWrappers objectForKey:uuid];
    if (wrapper == nil) {
    ...
    // Lấy về conversation
    [wrapper.client
     getConversationWithConversationId:conversationId
     completionHandler:^(BOOL status, int code, NSString *message, StringeeConversation *conversation) {
        if (!conversation) {
            callback(@[ @(NO), @(-3), @"Conversation not found.", [NSNull null] ]);
            return;
        ...
        [conversation
         getLocalMessagesWithCount:count
         completionHandler:^(BOOL status, int code, NSString *message, NSArray<StringeeMessage *> *messages) {
            callback(@[
                @(status), @(code), message, [RCTConvert StringeeMessages:[[messages reverseObjectEnumerator] allObjects]]
            ]);
        }];
    }];
}

RCT_EXPORT_METHOD(getLastMessages
                  : (NSString *)uuid conversationId
                  : (NSString *)conversationId count
                  : (NSUInteger)count loadDeletedMessage
                  : (BOOL)loadDeletedMessage loadDeletedMessageContent
                  : (BOOL)loadDeletedMessageContent callback
                  : (RCTResponseSenderBlock)callback) {
    RNClientWrapper *wrapper = [RNStringeeInstanceManager.instance.clientWrappers objectForKey:uuid];
    if (wrapper == nil) {
        callback(@[ @(NO), @(-1), @"Wrapper is not found" ]);
    ...
    [wrapper.client
     ...
     completionHandler:^(BOOL status, int code, NSString *message, StringeeConversation *conversation) {
        ...
        [conversation
         getLastMessagesWithCount:count
         loadDeletedMessage:loadDeletedMessage
         loadDeletedMessageContent:loadDeletedMessageContent
         completionHandler:^(BOOL status, int code, NSString *message, NSArray<StringeeMessage *> *messages) {
            callback(@[
                @(status), @(code), message, [RCTConvert StringeeMessages:[[messages reverseObjectEnumerator] allObjects]]
            ]);
...
RCT_EXPORT_METHOD(getAllLastMessages
                  : (NSString *)uuid conversationId
                  : (NSString *)conversationId count
                  : (NSUInteger)count loadDeletedMessage
                  : (BOOL)loadDeletedMessage loadDeletedMessageContent
...
RCT_EXPORT_METHOD(getMessagesAfter
                  : (NSString *)uuid conversationId
                  : (NSString *)conversationId sequence
                  : (NSUInteger)sequence count
                  : (NSUInteger)count loadDeletedMessage
                  : (BOOL)loadDeletedMessage loadDeletedMessageContent
                  : (BOOL)loadDeletedMessageContent callback
                  : (RCTResponseSenderBlock)callback) {
    ...
    [wrapper.client getConversationWithConversationId:conversationId
        ...
        [conversation getMessagesAfter:sequence
                             withCount:count
                    loadDeletedMessage:loadDeletedMessage
             loadDeletedMessageContent:loadDeletedMessageContent
                     completionHandler:^(BOOL status, int code, NSString *message, NSArray<StringeeMessage *> *messages) {
...
RCT_EXPORT_METHOD(getAllMessagesAfter
                  : (NSString *)uuid conversationId
                  : (NSString *)conversationId sequence
                  : (NSUInteger)sequence count
                  : (NSUInteger)count loadDeletedMessage
...
RCT_EXPORT_METHOD(getMessagesBefore
                  : (NSString *)uuid conversationId
                  : (NSString *)conversationId sequence
                  : (NSUInteger)sequence count
                  : (NSUInteger)count loadDeletedMessage
    ...
    [wrapper.client
     ...
     completionHandler:^(BOOL status, int code, NSString *message, StringeeConversation *conversation) {
        ...
        [conversation getMessagesBefore:sequence
                              withCount:count
                     loadDeletedMessage:loadDeletedMessage
              loadDeletedMessageContent:loadDeletedMessageContent
                      completionHandler:^(BOOL status, int code, NSString *message, NSArray<StringeeMessage *> *messages) {
...
RCT_EXPORT_METHOD(getAllMessagesBefore
                  : (NSString *)uuid conversationId
                  : (NSString *)conversationId sequence
                  : (NSUInteger)sequence count
                  : (NSUInteger)count loadDeletedMessage
...
RCT_EXPORT_METHOD(getMessageById
                  : (NSString *)uuid convId
                  : (NSString *)convId msgId
                  : (NSString *)msgId callback
                  : (RCTResponseSenderBlock)callback) {
    RNClientWrapper *wrapper = [RNStringeeInstanceManager.instance.clientWrappers objectForKey:uuid];
    if (wrapper == nil) {
    ...
    // Lấy về conversation
    [wrapper.client getConversationWithConversationId:convId
                                    completionHandler:^(BOOL status, int code, NSString *message, StringeeConversation *conversation) {
        if (!conversation) {
            callback(@[ @(NO), @(-3), @"Conversation not found." ]);
            return;
...
RCT_EXPORT_METHOD(getChatProfile : (NSString *)uuid widgetKey : (NSString *)widgetKey callback : (RCTResponseSenderBlock)callback) {
    ...
    [wrapper.client getChatProfileWithKey:widgetKey
                               completion:^(BOOL status, int code, NSString *message, StringeeChatProfile *chatProfile) {
        callback(@[ @(status), @(code), message, [RCTConvert SXChatProfile:chatProfile] ]);
    }];
}

RCT_EXPORT_METHOD(getLiveChatToken
                  : (NSString *)uuid widgetKey
                  : (NSString *)widgetKey name
                  : (NSString *)name email
                  : (NSString *)email callback
                  : (RCTResponseSenderBlock)callback) {
    RNClientWrapper *wrapper = [RNStringeeInstanceManager.instance.clientWrappers objectForKey:uuid];
    if (wrapper == nil) {
        callback(@[ @(NO), @(-1), @"Wrapper is not found" ]);
    ...
    [wrapper.client generateTokenForCustomerWithKey:widgetKey
                                           username:name
                                              email:email
                                         completion:^(BOOL status, int code, NSString *message, NSString *token) {
        if (status) {
...
RCT_EXPORT_METHOD(updateUserInfo
                  : (NSString *)uuid name
                  : (NSString *)name email
                  : (NSString *)email avatar
                  : (NSString *)avatar phone
                  : (NSString *)phone callback
                  : (RCTResponseSenderBlock)callback) {
    RNClientWrapper *wrapper = [RNStringeeInstanceManager.instance.clientWrappers objectForKey:uuid];
    if (wrapper == nil) {
        callback(@[ @(NO), @(-1), @"Wrapper is not found" ]);
    ...
    if (phone != nil && phone.length > 0) {
        [wrapper.client updateUserInfoWithUsername:name
                                             email:email
                                            avatar:avatar
                                             phone:phone
                                        completion:^(BOOL status, int code, NSString *message) {
...
RCT_EXPORT_METHOD(updateUserInfo2 : (NSString *)uuid param : (NSString *)param callback : (RCTResponseSenderBlock)callback) {
    ...
    [wrapper.client updateUserInfo:newUserInfo
                        completion:^(BOOL status, int code, NSString *message) {
        callback(@[ @(status), @(code), message ]);
    }];
}
...
RCT_EXPORT_METHOD(getUserInfo : (NSString *)uuid userIds : (NSArray *)userIds callback : (RCTResponseSenderBlock)callback) {
    ...
    [wrapper.client getUserInfosByUserIds:userIds
                               completion:^(BOOL status, int code, NSString *message, NSArray<StringeeIdentity *> *users) {
...
RCT_EXPORT_METHOD(createLiveChatConversation : (NSString *)uuid queueId : (NSString *)queueId callback : (RCTResponseSenderBlock)callback) {
    ...
    [wrapper.client createLiveChatConversationWithQueueId:queueId
                                               completion:^(BOOL status, int code, NSString *message, StringeeConversation *conversation) {
        callback(@[ @(status), @(code), message, [RCTConvert StringeeConversation:conversation] ]);
    }];
}

RCT_EXPORT_METHOD(sendChatTranscript
                  : (NSString *)uuid email
                  : (NSString *)email convId
                  : (NSString *)convId domain
                  : (NSString *)domain callback
                  : (RCTResponseSenderBlock)callback) {
    RNClientWrapper *wrapper = [RNStringeeInstanceManager.instance.clientWrappers objectForKey:uuid];
    if (wrapper == nil) {
        callback(@[ @(NO), @(-1), @"Wrapper is not found" ]);
    ...
    [wrapper.client sendChatTranscriptTo:email
                                  convId:convId
                                  domain:domain
                              completion:^(BOOL status, int code, NSString *message) {
        callback(@[ @(status), @(code), message ]);
...
RCT_EXPORT_METHOD(endChat : (NSString *)uuid convId : (NSString *)convId callback : (RCTResponseSenderBlock)callback) {
    ...
    [wrapper.client endChatSupportWithConvId:convId
                                  completion:^(BOOL status, int code, NSString *message) {
        callback(@[ @(status), @(code), message ]);
    }];
}

RCT_EXPORT_METHOD(createTicketForMissedChat
                  : (NSString *)uuid widgetKey
                  : (NSString *)widgetKey name
                  : (NSString *)name email
                  : (NSString *)email phone
                  : (NSString *)phone note
                  : (NSString *)note callback
                  : (RCTResponseSenderBlock)callback) {
    RNClientWrapper *wrapper = [RNStringeeInstanceManager.instance.clientWrappers objectForKey:uuid];
    if (wrapper == nil) {
        callback(@[ @(NO), @(-1), @"Wrapper is not found" ]);
    ...
    if (phone != nil && phone.length > 0) {
        [wrapper.client createTicketForMissChatWithKey:widgetKey
                                              username:name
                                                 email:email
                                                 phone:phone
                                                  note:note
...
RCT_EXPORT_METHOD(createLiveChatTicket
                  : (NSString *)uuid widgetKey
                  : (NSString *)widgetKey name
                  : (NSString *)name email
                  : (NSString *)email phone
